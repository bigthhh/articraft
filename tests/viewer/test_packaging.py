...
def _bundle() -> tuple[bytes, str]:
    ...
    return packaging.pack_artc(manifest=manifest, files=files, key=key), packaging.public_key_b64(key)
...
def test_roundtrip_verifies_as_own() -> None:
    bundle, pub = _bundle()
    report = packaging.verify_artc(bundle, own_public_key_b64=pub)
...
def test_tampered_payload_fails_integrity() -> None:
    ...
    report = packaging.verify_artc(tampered, own_public_key_b64=pub)
...
def test_tampered_manifest_fails_signature() -> None:
    ...
    report = packaging.verify_artc(tampered)
...
def test_foreign_key_verifies_but_not_own() -> None:
    ...
    report = packaging.verify_artc(bundle, own_public_key_b64=other_pub)
